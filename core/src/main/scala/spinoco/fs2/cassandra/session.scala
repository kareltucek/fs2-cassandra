      ...
      Ref.of[F, SessionState](SessionState(Map.empty)) map { state =>
        ...
        def executeDML[I, R](statement: DMLStatement[I, R],o: DMLOptions,i: I): F[R] =
          mkStatement(statement,i).flatMap { bs =>
            Sync[F].suspend(cqlSession.executeAsync(Options.applyDMLOptions(bs,o)).toF).flatMap { rs =>
        ...
        new CassandraSession[F] {
          ...
          def executeRaw[T <: Statement[T]](statement: T): F[AsyncResultSet] = Sync[F].suspend(cqlSession.executeAsync(statement).toF)
          def executeCql(cql: String, o: DMLOptions = Options.defaultDML): F[AsyncResultSet] = Sync[F].suspend(cqlSession.executeAsync(cql).toF)
          ...
          def prepareCql(cql: String): F[PreparedStatement] = Sync[F].suspend(cqlSession.prepareAsync(cql).toF)
