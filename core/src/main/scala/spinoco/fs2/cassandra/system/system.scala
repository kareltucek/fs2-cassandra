...
package object system {
  ...
  def samePrimaryKey(current: TableMetadata, desired: AbstractTable[_,_,_,_]):Boolean = {
    val currentPk = current.getPartitionKey.asScala.map(_.getName.toString.toLowerCase)
    val currentCk = current.getClusteringColumns.asScala.keys.map(_.getName.toString.toLowerCase)
    ...
    maybeCurrent match {
      ...
      case Some(current) =>
        ...
        if (desiredTable.name != current.getName) Nil
          ...
          val currentColumns =
            current.getColumns.asScala.map { c => c._1.toString.toLowerCase -> c._2.getType }
  ...
  def migrateMaterializedView(desiredView: MaterializedView[_,_,_], maybeCurrent: Option[ViewMetadata]): Seq[String] = {
...
//        if (desiredView.name != current.getName) Nil
//        else if (!samePrimaryKey2(current.getPartitionKey.asScala.toList, current.getClusteringColumns.asScala.toMap, desiredView.table)) desiredView.cqlStatement
//        else if (!samePrimaryKey(current, desiredView) || !sameColumns) s"DROP MATERIALIZED VIEW ${desiredView.fullName}" +: desiredView.cqlStatement
//        else Nil
    }
  }
