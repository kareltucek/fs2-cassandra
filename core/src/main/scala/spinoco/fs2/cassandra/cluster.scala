...
//
  ...
  def wrap[F[_] : Async : ContextShift](session: CassandraSession[F]): CassandraCluster[F] = {
      new CassandraCluster[F] {
        def session: Resource[F, CassandraSession[F]] = session.flatMap(cs => Resource.make(Sync[F].pure(cs))(_ => Sync[F].unit))
      }
  }
}
